...
export class CoordinatorDO implements DurableObject {
  ...
  private async scaleDown(region: string): Promise<void> {
    ...
    // Note: The actual shard will shut down when connections reach 0
    // This is handled by the shard itself checking its status periodically
  }
  ...
  private scheduleHealthCheck(): void {
    ...
    // In production, use DO alarms for more reliable scheduling
    const checkInterval = setInterval(() => {
      this.performHealthCheck();
    }, 30000); // Every 30 seconds

    // Note: In production, you should use this.state.storage.setAlarm()
    // instead of setInterval for better reliability
  }
