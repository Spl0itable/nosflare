...
function buildCountQuery(filter: NostrFilter): { sql: string; params: any[] } {
  ...
  if (directTags.length > 0 && otherTags.length === 0) {
    // Use event_tags_cache_multi to count ALL matching tags (not just first occurrence)
    // This is critical for reply events which have multiple e-tags
...
function buildQuery(filter: NostrFilter): { sql: string; params: any[] } {
  ...
  if (directTags.length > 0 && otherTags.length === 0) {
    ...
    // Use event_tags_cache_multi to find ALL matching tags (not just first occurrence)
    // This is critical for reply events which have multiple e-tags
    if (directTags.length === 1) {
