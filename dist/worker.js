...
var blockedContent = /* @__PURE__ */ new Set(["~~ hello world! ~~"]);
...
var pubkeyRateLimiter2 = new rateLimiter(10 / 6e4, 10);
  ...
  try {
    if (deletionEvent.kind === 5 && deletionEvent.pubkey) {
      ...
      for (const eventId of deletedEventIds) {
        const existingEvent = await env.relayDb.prepare("SELECT * FROM events WHERE id = ?").bind(eventId).first();
        if (existingEvent && existingEvent.pubkey === deletionEvent.pubkey) {
          await env.relayDb.prepare("DELETE FROM events WHERE id = ?").bind(eventId).run();
...
async function saveEventToD1(event, env) {
  try {
    ...
    const existingEvent = await env.relayDb.prepare("SELECT * FROM events WHERE id = ?").bind(event.id).first();
    if (existingEvent) {
    ...
    await env.relayDb.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(event.id, event.pubkey, event.created_at, event.kind, JSON.stringify(event.tags), event.content).run();
...
async function processReq(message, server, env) {
  ...
  try {
    const events = await env.relayDb.prepare(query).bind(...params).all();
