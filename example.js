  ...
  try {
    ...
    const tagPromises = event.tags.map(async (tag) => {
      if (tag[0] === "e") {
        ...
        return relayDb.put(eTagKey, JSON.stringify(event)).then(() => relayDb.put(eTagCountKey, (eTagCount + 1).toString()));
      } else if (tag[0] === "p") {
        const pTagCountKey = `${PTAG_COUNT_KEY_PREFIX}${tag[1]}`;
        const pTagCount = parseInt(await relayDb.get(pTagCountKey, "text") || "0", 10);
        const pTagKey = `p-${tag[1]}:${pTagCount + 1}`;
        return relayDb.put(pTagKey, JSON.stringify(event)).then(() => relayDb.put(pTagCountKey, (pTagCount + 1).toString()));
      }
    });
    await Promise.all([
