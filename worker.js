...
async function archiveOldEvents(db, r2) {
  ...
  `).bind(...eventIds).all();
  const tagsByEvent = /* @__PURE__ */ new Map();
  for (const tag of allTags.results || []) {
    const eventId = tag.event_id;
...
var _RelayWebSocket = class _RelayWebSocket {
  ...
  async webSocketMessage(ws, message) {
    ...
    if (sessionAge > _RelayWebSocket.MAX_CONNECTION_DURATION) {
      console.log(`Session ${attachment.sessionId} expired (age: ${Math.round(sessionAge / 1e3 / 60)} minutes)`);
  ...
  async webSocketClose(ws, code, reason, wasClean) {
    ...
    if (attachment) {
      const sessionAge = Date.now() - attachment.connectedAt;
      const sessionMinutes = Math.round(sessionAge / 1e3 / 60);
