...
async function processDeletionEvent(event, env) {
  ...
  const ownedEventIds = existingEvents.results.filter((e) => e.pubkey === event.pubkey).map((e) => e.id);
...
var _RelayWebSocket = class _RelayWebSocket {
  ...
  async sendToSpecificDO(doName, event) {
    try {
      ...
      let locationHint = _RelayWebSocket.ENDPOINT_HINTS[doName];
      if (!locationHint) {
